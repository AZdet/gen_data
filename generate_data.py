...
def unionBBox(bbox1, bbox2, low=0.2, high=0.9):
...
def generateDataRandomMask(n_iter, save_dir):
    ...
    for path1 in gen1:
        ...
        while True:
            ...
            out = genOccluded(target_mask, target_img, mask, mode)
            if out is None:
                path1 = next(gen1)
            else:
                break
...
def get_random_mask(mask1):
    ...
    if rand() < 0.5:
        ...
        width = randint(5, 15)
        height = randint(40, 127)
        x = randint(10,110)
        mask[-height:-1, x:x+width] = 1
    else:
        masks = ['random.png', 'random2.png', 'random3.png','random4.png', 'random5.png']
        length = len(masks)
        idx = randint(length)
...
if __name__ == "__main__":
    for mode in ['p2c']:
        generateData(mode, 100000, 'train')
